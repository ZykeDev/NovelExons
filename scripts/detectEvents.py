...
class Vertex:
    ...
    def __str__(self):
        return str(self.id) + ' {' + str(self.type) + '} adjacent: ' + str([(x.id, self.get_weight(x)) for x in self.adjacent])
...
def buildMEMsGraph(refPath, e_memsPath, i_memsPath, gtfPath):
    ...
    for line in open(e_memsPath, 'r').readlines():
        ...
        # Ignore mems without that didn't find a text
        if exon_text != "":
    ...
    # Init MEMs graphs
    g = Graph()         # Exon graph
    ig = Graph()        # Intron graph
    # Add a Start and an End as vertices
        ...
        # Case 2 [e, i, e]
            ...
            for (eID1, eID2, iID) in ex_in:
                if id1 == eID1 and id2 == eID2:
                    ...
                    intron_text = getTextFromID(IntrBitV, intrText, iID)
                            ...
                            if imr and not linked:
                                ...
                                if iID == IntrBitV.rank(im[0] - 1):
                                    ...
                                    while newGap > K2 and stop > 0:
                                        ...
                                        nextLeftOverlap = maxOverlap(iread, intron_text[nextMem[0] : nextMem[0] + nextMem[2]])
                                        nextRightOverlap = maxOverlap(intron_text[nextMem[0] : nextMem[0] + nextMem[2]], exon2_text)
...
def insertMEMs(g, mems, BitV, text, adjm, mtype):
    ...
    for mlist in mems:
        for mr in mlist:
            if mr:
                ...
                # START
                if startInfo[0] or BitV.rank(m[0] - 1) == 1:
                    if isNew(g, m):
                        # If the corresponding exon doesn't have a father
                        ID = BitV.rank(m[0] - 1)
                        if parents[ID] == []:
                            # Add the new node and link it to the start
                            g.add_edge("start", m, err, "Start", mtype)

                            # Check if the read has an intron prefix
                ...
                while nextP <= maxRead:
                    for mr2 in mems[nextP]:
                        ...
                        if isNew(g, m2):
                            # True if m & m2 are from the same exon, False otherwise
                            linkageInfo = checkMEMs(adjm, m, m2, BitV, text, read)
                            flag = linkageInfo[0]
                            err = linkageInfo[1]
                            
                            if err >= 0:
                                if not flag and isNew(g, m2):
                                    g.add_vertex(m2, mtype)

                                if flag:
                                    g.add_edge(m, m2, err, mtype, mtype)
                                    isExt = True
                                    isNov = True
                                else:
                                    isNov = True
                ...
                if not isExt and not isNov: 
                    ...
                    if endFlag:
                        g.add_edge(m, "end", err, mtype, "End")
...
# Computes the max overlap between 2 strings
def maxOverlap(s1, s2):
    maxoverlap = 0

    for k in range(len(s1)):
        if overlap(s1, s2, k):
            thisOverlap = len(s1) - k
            if maxoverlap < thisOverlap:
...
def strToMem(string):
    ...
    mem = tuple([int(x) for x in string.split(",")])
    return mem


def getTextFromID(BitV, text, mID):
    s = BitV.select(mID)
    e = BitV.select(mID+1)

    # Test if the mem is the last in the BitV
    try:
        t = text[s : e-1]
    except:
        # If it is, return the text till the last base (exclude the final "|")
        t = text[s : -1]
...
def checkMEMs(adjm, mem1, mem2, BitV, text, read):
    (m1t, m1p, m1l) = mem1
    (m2t, m2p, m2l) = mem2
...
# python3 ./scripts/detectEvents.py -g input/Homo_sapiens.GRCh38.dna.chromosome.21.fa -a input/ENSG00000279493.no2.gtf -m out/case_3.ENST00000624081_e_14988_21_5012632.exons.mem -o out/output.events.csv
